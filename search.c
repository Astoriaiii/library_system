...
BookList find_book_by_title (const char *title) {
    ...
    while (title_Book != NULL) {
        ...
        if (strcmp(title_Book->title, title) == 0){
            ...
            printf("%d\t%s\t%s\t%d\t%d\t\n", title_Book->id, title_Book->title, title_Book->authors, title_Book->year, title_Book->copies);
            index ++;
        }
        //if (index > 0){
            //break;
...
//printf("1");
	...
	//printf("%s %s\n", author,author_Book ->authors);
        if (strcmp(author_Book->authors, author) == 0){
            //printf ("ID\tTitle\tAuthor\tYear\tCopies\t");
            printf("%d\t%s\t%s\t%d\t%d\t\n", author_Book->id, author_Book->title, author_Book->authors, author_Book->year, author_Book->copies);
            index ++;
        }
       // if (index > 0){
            //break;
...
BookList find_book_by_year (unsigned int year) {
    ...
    while (year_Book) {
        if (year_Book->year == year){
            //printf ("ID\tTitle\tAuthor\tYear\tCopies\t\n");
            printf("%d\t%s\t%s\t%d\t%d\t\n", year_Book->id, year_Book->title, year_Book->authors, year_Book->year, year_Book->copies);
            index ++;
        }
        //if (index > 0){
            //break;
