...
//return 0 if no username, 1 if wrong password, 2 if common user, 3 if special user
int UserLogging(char* username, char* password);

//allow users to borrow book
//return 0 if no this book, 1 if the user already had the book, 2 if successful
int UserBorrowBook(User* user, unsigned int id);

//allow users to return book
//return 0 if no this book, 1 if the user had not borrow this book, 2 if successful
